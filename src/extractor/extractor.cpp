...
KeyPoints Extracotr::postProcess( const Config& config, std::vector<Ort::Value> tensor )
{
...
std::pair<KeyPoints, std::vector<Region>> Extracotr::distributeKeyPointsDebug( const KeyPoints& key_points, const cv::Mat& image )
{
...
KeyPoints Extracotr::distributeKeyPoints( const KeyPoints& key_points, const cv::Mat& image )
{
  INFO( logger, "Distributing key points" );
...
KeyPoints Extracotr::inferenceImage( const Config& config, const cv::Mat& image )
{
  INFO( logger, "**** Inferencing image ****" );
