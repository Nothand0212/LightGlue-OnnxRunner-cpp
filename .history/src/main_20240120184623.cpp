...
void visualizeMatches( const cv::Mat& src, const cv::Mat& dst, const std::vector<cv::Point2f>& keypoints1, const std::vector<cv::Point2f>& keypoints2, const std::pair<std::vector<cv::Point2f>, std::vector<cv::Point2f>>& keypoints )
{
...
int main( int argc, char const* argv[] )
{
  ...
  for ( ; iter_src != image_src_mat_vec.end(); ++iter_src, ++iter_dst )
  {
    ...
    visualizeMatches( *iter_src, *iter_dst, key_points_src, key_points_dst, key_points_result );
