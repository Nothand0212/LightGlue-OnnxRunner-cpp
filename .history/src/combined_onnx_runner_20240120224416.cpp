...
int CombinedOnnxRunner::inference( const Config& config, const cv::Mat& image_src, const cv::Mat& image_dst )
{
  ...
  try
  {
    ...
    INFO( logger, "create memory info handler" );
    // auto                    memory_info_handler = Ort::MemoryInfo::CreateCpu( OrtAllocatorType::OrtDeviceAllocator, OrtMemType::OrtMemTypeCPU );
    auto                    memory_info_handler = Ort::MemoryInfo::CreateCuda( OrtAllocatorType::OrtDeviceAllocator, OrtMemType::OrtMemTypeDefault, 0 );
...
int CombinedOnnxRunner::postProcess( const Config& config )
{
  try
  {
    ...
    for ( int i = 0; i < matches_0_shape[ 0 ] * 2; i += 2 )
    {
      ...
      {
        // logger->info( "matche pair index: [{0}, {1}] score: {2}", matches_0_ptr[ i ], matches_0_ptr[ i + 1 ], match_scores_0_ptr[ count ] );
